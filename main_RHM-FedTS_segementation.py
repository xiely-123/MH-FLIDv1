...
import model_segmentation.Unet_Plus as Unet_Plus
import model_segmentation.Unet as Unet
import model_segmentation.ResUnet as ResUnet
import model_segmentation.FCN as FCN



from untils.utils import average_weights, exp_details
from untils.utils_iid_noiid import get_dataset_seg
...
if __name__ == '__main__':
    ...
    # BUILD MODEL
    num_classes = 2
    global_model_0 = Unet_Plus.NestedUNet(3,num_classes)
    global_model_1 = Unet.UNet(3,num_classes)
    global_model_2 = ResUnet.ResUNet(3,num_classes)
    global_model_3 = FCN.FNC_8S(3,num_classes)
...
# training
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            train_dataset, test_dataset = get_dataset_seg(idx,args.task)
        ...
        # if (iter+1) % 1 == 0:
            # for idx in idxs_users:
                ...
                # net_local_test = copy.deepcopy(net_local_list[0])
                # local.inference(net_local_test,idx=idx, task=args.task)
                # net_local_test = copy.deepcopy(net_local_list[1])
                # local.inference(net_local_test,idx=idx, task=args.task)
                # net_local_test = copy.deepcopy(net_local_list[2])
                # local.inference(net_local_test,idx=idx, task=args.task)
                # net_local_test = copy.deepcopy(net_local_list[3])
                # local.inference(net_local_test,idx=idx, task=args.task)
